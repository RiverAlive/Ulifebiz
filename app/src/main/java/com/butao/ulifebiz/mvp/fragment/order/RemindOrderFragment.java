...
import com.butao.ulifebiz.mvp.dialog.RemindDialog;
 ...
 * 功能描述 ：催单
 */
public class RemindOrderFragment extends MvpFragment<RefoundOrderPresenter> implements RefundOrderView {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        pageNum = 0;
        initRFView();
        mvpPresenter.loadRMOrder(pageNum);
        return view;
    }
...
//            ToastUtils.showShortToast("催单申请提交成功，请您耐心等待");
            recycleAdapter.getDatas().remove(select);
            emptyWrapper.notifyDataSetChanged();
            if(recycleAdapter!=null) {
                OrderRemindActivity.remindActivity.rfTabTitle(recycleAdapter.getItemCount(),0);
            }
        } else {
...
//                            textView.setEllipsize(TextUtils.TruncateAt.END);
                        ...
                        holder.setVisible(R.id.txt_ignore,true);
                        holder.setText(R.id.txt_receive,"催单");
                        ...
                        holder.setOnClickListener(R.id.txt_receive, new View.OnClickListener() {
                            ...
                            public void onClick(View v) {//回复
                                ...
                                remindDialog.dialogShow();
                                remindDialog.setRemindlistener(new RemindDialog.Remindlistener() {
                                    @Override
                                    public void Remind(String remark) {
                                        select=position;
                                        RefundOrderModel.RFOrderModel rfOrderModel= (RefundOrderModel.RFOrderModel)   recycleAdapter.getDatas().get(position);
                                        if(!TextUtils.isEmpty(rfOrderModel.getUrgeId())){
                                            mvpPresenter.loadRemind(rfOrderModel.getUrgeId(),"1",remark);
                                        }
                                    }
                                });
                            }
                        });
                        holder.setOnClickListener(R.id.txt_ignore, new View.OnClickListener() {//忽略
                            @Override
                            public void onClick(View v) {
                                select=position;
                                mvpPresenter.loadRemind(rfOrderModel.getUrgeId(),"-1",null);
                            }
                        });
                    }
                }
            ...
            emptyWrapper.setEmptyView(LayoutInflater.from(getActivity()).
                    inflate(R.layout.view_empty_util, ryRfOrder, false), getResources().getDrawable(R.mipmap.cuidantixing_no), "没有退款提醒\nNo reminder");
