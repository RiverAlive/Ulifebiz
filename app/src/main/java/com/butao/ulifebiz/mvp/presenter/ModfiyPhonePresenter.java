 ...
 * 功能描述 ：
 */
public class ModfiyPhonePresenter extends BasePresenter<ModifyPhoneView> {
    public ModfiyPhonePresenter(ModifyPhoneView view){
        attachView(view);
    }
    /**
     * 48.修改手机号
     */
    public void loadModifyPhone(String phone){
        mvpView.showLoading();
        Map<String,String > map = new HashMap<>();
        map.put("token", CApplication.getIntstance().getToken());
        map.put("phone", phone);
        addSubscription(apiServices.loadModifyPhone(map),
                new ApiCallback<BaseModel>() {
                    @Override
                    public void onSuccess(BaseModel model) {
                        mvpView.getModifyPhoneSuccess(model);
                    }
