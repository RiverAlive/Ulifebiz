...
public class StarBarView extends View {
    ...
    public void setStarMark(float mark){
        if (integerMark) {
            starMark = (int) Math.ceil(mark)/2;
        }else {
            starMark = Math.round(mark * 10) * 1.0f / 20;
        }
        if (this.onStarChangeListener != null) {
            this.onStarChangeListener.onStarChange(starMark*2);  //调用监听接口
        }
        invalidate();
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch(event.getAction()){
            case MotionEvent.ACTION_DOWN: {
                if(!click) {
                    setStarMark((x * 1.0f / (getMeasuredWidth() * 1.0f / starCount)) * 2);
                }
                break;
            }
